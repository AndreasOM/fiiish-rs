...
use oml_game::math::{Matrix22, Vector2};
...
impl OverlapChecker {
	...
	fn any_points_in_polygon(
		points_a: &Vec<Segment>,
		polygon_b: &Vec<Segment>,
		_debug_renderer: &Option<RefCell<DebugRenderer>>,
	) -> bool {
		...
		for sa in points_a.iter() {
			...
			for sb in polygon_b.iter() {
				let side =
					OverlapChecker::get_side(&sb.start, &sb.end, &p, &None /*debug_renderer*/);
	...
	fn do_sub_shapes_overlap(
		a: &OverlapCheckerSubItem,
		b: &OverlapCheckerSubItem,
		debug_renderer: &Option<RefCell<DebugRenderer>>,
	) -> bool {
		...
		let any_inside =
			OverlapChecker::any_points_in_polygon(&segments_a, &segments_b, debug_renderer)
	...
	pub fn do_shapes_overlap(
		a: &OverlapCheckerItem,
		b: &OverlapCheckerItem,
		debug_renderer: &Option<RefCell<DebugRenderer>>,
	) -> bool {
		...
		for ssa in a.shape.sub_shape_iter() {
			...
			for ssb in b.shape.sub_shape_iter() {
				...
				let maybe_dr = if n < 100 { debug_renderer } else { &None };
