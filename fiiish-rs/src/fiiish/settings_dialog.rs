...
impl UiElement for SettingsDialog {
	...
	fn setup_within_container(&mut self, container: &mut UiElementContainerData) {
		let mut background =
			container.add_child_element(UiImage::new("screen_frame_2", &self.size));
				...
				// center box with actual results
				{
					...
					)); // left space
					{
						...
						let vbox = parent.add_child_element(vbox);
						{
							let mut parent = vbox.borrow_mut();
							{
								let text_space = parent.add_child_element(UiSpacer::new(
									&Vector2::new(620.0, 284.0),
									&Color::from_rgba(0.2, 0.2, 0.8, 0.5),
								)); // placeholder
	// 284
								...
								let vbox = parent.add_child_element(vbox);
								{
									...
									let mut l =
										UiLabel::new(&Vector2::new(620.0, 64.0), "Fiiish! RS");
							...
							}
							{
								...
								let music_togglebutton =
									hbox.add_child_element(UiToggleButton::new(
										"button_music_on",
										"button_music_off",
	...
	fn update(&mut self, _container: &UiElementContainerData, _time_step: f64) {
		...
		if let Some(music_togglebutton) = &mut self.music_togglebutton {
			...
			match music_togglebutton
				.as_any_mut()
				.downcast_mut::<UiToggleButton>()
			{
				Some(mtb) => {
