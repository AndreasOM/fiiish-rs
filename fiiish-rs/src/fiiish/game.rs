...
use crate::{OverlapChecker, OverlapCheckerItem};
...
impl Game {
	...
	pub fn setup(&mut self, system: &mut System, renderer: &mut Renderer) {
		...
		self.entity_configuration_manager
			.load(system, "entity_config.whatever");
		...
		self.shape_cache
			.load_shape(system, "fish_swim", EntityId::FISHSWIM);
		self.shape_cache
			.load_shape(system, "block-1x1", EntityId::BLOCK1X1);

		self.shape_cache
			.load_shape(system, "rock-a", EntityId::ROCKA);
		self.shape_cache
			.load_shape(system, "rock-b", EntityId::ROCKB);
		self.shape_cache
			.load_shape(system, "rock-c", EntityId::ROCKC);
		self.shape_cache
			.load_shape(system, "rock-d", EntityId::ROCKD);
		self.shape_cache
			.load_shape(system, "rock-e", EntityId::ROCKE);
		self.shape_cache
			.load_shape(system, "rock-f", EntityId::ROCKF);

		self.shape_cache
			.load_shape(system, "seaweed-a", EntityId::SEAWEEDA);
		self.shape_cache
			.load_shape(system, "seaweed-b", EntityId::SEAWEEDB);
		self.shape_cache
			.load_shape(system, "seaweed-c", EntityId::SEAWEEDC);
		self.shape_cache
			.load_shape(system, "seaweed-d", EntityId::SEAWEEDD);
		self.shape_cache
			.load_shape(system, "seaweed-e", EntityId::SEAWEEDE);
		self.shape_cache
			.load_shape(system, "seaweed-f", EntityId::SEAWEEDF);
		self.shape_cache
			.load_shape(system, "seaweed-g", EntityId::SEAWEEDG);
		...
		let mut p = Fish::new();
		let ec = self
			.entity_configuration_manager
			.get_config(EntityId::FIIISH as u32);
		p.setup(&ec);
		self.fishes.push(p);
							...
							if let Some(fish_shape) = self.shape_cache.find(EntityId::FISHSWIM) {
								...
								if let Some(obstacle_shape) = self.shape_cache.find(o.entity_id()) {
									...
									*/
									...
									if OverlapChecker::do_shapes_overlap(
										&a,
										&b,
										&*self.debug_renderer,
									) {
	...
	pub fn update(&mut self, wuc: &mut WindowUpdateContext, auc: &mut AppUpdateContext) {
				...
				match self.state {
					GameState::None => {
						...
						if self.distance > 0.0 {
							self.player
								.add_to_last_distance(self.distance.floor() as u32);
							self.distance = 0.0;
						}
						//						self.coins = 0;
						//						self.distance = 0.0;
					...
					GameState::Die => {
						...
						self.player
							.log_play(self.coins, self.distance.floor() as u32);
					},
					GameState::Dead => {
						if p.can_respawn() {
		...
		let mut euc = EntityUpdateContext::new().set_time_step(wuc.time_step);
		...
		if wuc.was_key_pressed('r' as u8) {
			//			self.spawn_pickups();
			self.zone_manager
				.spawn_pickups(&self.entity_configuration_manager, &mut self.entity_manager);
		}
	...
	fn take_distance(&mut self, amount: u32) -> u32 {
		...
		let d = if amount > fd { fd } else { amount };
