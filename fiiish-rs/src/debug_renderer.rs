...
//			println!("{} + {} = {}", l.start.y, vl.y, v2.y );
	...
	pub fn add_line( &mut self, start: &Vector2, end: &Vector2, width: f32 ) {
		let line = { Line { start: *start, end: *end, width } };
		self.lines.push( line );
	}
	pub fn add_frame( &mut self, pos: &Vector2, size: &Vector2, width: f32 ) {
		let half_size = size.scaled_vector2( &Vector2::new( -0.5, 0.5 ) );
		let top_left = pos.add( &half_size );
		let bottom_right = pos.sub( &half_size );

		self.add_line( &top_left, &bottom_right, width );
		self.add_line( &Vector2::new( bottom_right.x, top_left.y ), &Vector2::new( top_left.x, bottom_right.y ), width );
		self.add_line( &Vector2::new( top_left.x, top_left.y ), &Vector2::new( top_left.x, bottom_right.y ), width );
		self.add_line( &Vector2::new( bottom_right.x, top_left.y ), &Vector2::new( bottom_right.x, bottom_right.y ), width );
		self.add_line( &Vector2::new( top_left.x, top_left.y ), &Vector2::new( bottom_right.x, top_left.y ), width );
		self.add_line( &Vector2::new( top_left.x, bottom_right.y ), &Vector2::new( bottom_right.x, bottom_right.y ), width );
	}
}
